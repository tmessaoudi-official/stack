...
| 47 | Developer request 2026-09-14: fix the 04android boot warnings/deprecations (~380 avdmanager package.xml/devices.xml lines from the unversioned cmdline-tools on PATH, the sdkmanager --licenses deprecation, dead legacy PATH entries, emulator/telemetry notices). PLAN ONLY, not implemented — see docs/plans/android-boot-warnings.plan.md. | M | todo | - | docker/config/dist/bin/android-bin/** docker/config/dist/bin/alltogether/** templates/shell/profile.sh bin/tests/startup-prologue.test.sh docs/** |
<!-- /progress-block -->
### Blocked
