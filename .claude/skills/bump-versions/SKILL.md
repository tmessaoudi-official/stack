...
5. **What each change needs + rebuild command**: do NOT advise deleting a `tools/versions/` marker or setting `GLOBAL_STACK_RELOAD_<RUNTIME>=true` — a pin bump needs neither. Every runtime-installed tool is gated by `gs_version_gate`, which sees the new pin on the next start, WARNs and reinstalls it (either direction). The `RELOAD_NODE*` / `_JAVA*` / `_FLUTTER3` switches do not even wipe (CLAUDE.md § Gotchas). Sort the changed pins into two groups and say which applies:
