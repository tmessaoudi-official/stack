...
- **`env-scan.sh` silently deletes `GLOBAL_STACK_LOCAL_*` vars**: env-scan rebuilds `.env.local` from `.env` as sole source — any key present only in `.env.local` (not in `.env`) is dropped on every run. After any `env-scan.sh` run: `cat .env.local-only >> .env.local` (or `make sync-local`). Keep `.env.local-only` as the authoritative LOCAL_ key registry.
