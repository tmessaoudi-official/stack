...
- **`android`'s `--sdk` is a GLOBAL option — `android --sdk=<path> sdk install …`, never `android sdk install --sdk=…`.** The second form is rejected outright: `Unknown option: '--sdk=…'`, exit **2**, for `sdk install` and `sdk list` alike [Verified 2026-09-11 against `android 1.0.15985488`, the build the pinned `commandlinetools-linux-${GLOBAL_STACK_ANDROID_SDK_BUILD}_latest.zip` yields — so a clean machine reproduces it]. `b2ae4d1` shipped the wrong position at all three call sites in `global-stack-android-setup.sh` and nothing noticed for a day, because `gs_version_gate` returns `skip` on a warm `tools/` volume (`tools/` is a host bind mount, so `make hard-restart` does not clear it, and `make down` does not clear `versions/`) — the block had literally never run. It was armed, not harmless: `global-stack-android-start.sh:119` `sudo rm -rf`s `ANDROID_HOME` **before** calling setup, so any of `RELOAD_ANDROID=true`, a bump of the three pinned SDK vars, a missing `android.sdkmanager` marker, a clean machine or `make soft-restart` would have deleted a working SDK and then failed on the first command of the reinstall. Two more facts worth keeping, both measured: `android sdk list` with no `--all` lists **installed only**, so it is a real verification surface; and its id column is the install id with `;`→`/` **except `platform-tools`**, which is single-instance upstream and is listed **bare** with its version in column 2 — so the obvious `;`→`/` transform of the install array makes a correct install report itself missing. `startup-prologue.test.sh` §43g/§43g2 pin both directions and §43p-43t run the shipped block against a stub modelled on the real CLI.
