...
- **Makefile portability tests**: `bash bin/tests/makefile-posix.test.sh` — 5 checks. Recipes run under `/bin/sh` (dash) because the Makefile never sets `SHELL`, so `source` (exit 127, and the rest of an `&&` line never runs) and `&>` (backgrounds instead of silencing) are fatal in recipes. Also asserts every `make <target>` named in a recipe message actually exists, reading the target list from make's own database so `$(eval $(call ...))` targets count
