...
# ═══════════════════════════════════════════════════════════════════════════
# Section 109 — 5xx transient retry (Fix 1) + codeberg fallback resilience (Fix 2)
# ═══════════════════════════════════════════════════════════════════════════
section "109 — 5xx transient retry + codeberg fallback resilience"

# Helper: create a fake curl binary in a tmpdir that returns a given HTTP status.
# The binary writes the status code to stdout (as curl -w "%{http_code}" would), writes
# nothing to the body file, and exits 0 (simulating a server that responded with that status).
# A counter file tracks how many times the binary has been called so we can simulate
# "N failures then success" by checking the count.
#
# Usage: _make_fake_curl <dir> <status_on_calls_1_to_N> <N> [<success_body>]
# Creates: <dir>/curl  (executable fake curl)
# The fake curl:
#   - reads the call count from <dir>/.call_count
#   - if count <= N: returns the failure status (body empty)
#   - if count >  N: returns 200 (body = success_body written to -o file)

# t109a: _gs_eu2_http_get_core retries on 503 and succeeds after transient failure
t "t109a: http_get_core retries 503 and recovers on 2nd attempt" bash -c "
    source '/stack/bin/lib/env-update/http/curl.sh'
    # Create a fake curl that fails with 503 on attempt 1, succeeds on attempt 2.
    _fake_dir=\"\${TMP_DIR}/t109a_curl\"
    mkdir -p \"\${_fake_dir}\"
    _count_file=\"\${_fake_dir}/.call_count\"
    printf '0' > \"\${_count_file}\"
    cat > \"\${_fake_dir}/curl\" <<'FAKECURL'
#!/bin/bash
# Fake curl: 503 on first call, 200 on subsequent calls
_count_file=\"\$(dirname \"\$0\")/.call_count\"
_c=\"\$(cat \"\${_count_file}\" 2>/dev/null || echo 0)\"
_c=\$(( _c + 1 ))
printf '%d' \"\${_c}\" > \"\${_count_file}\"
# Parse -o and -w flags from args
_out_file=''
_want_w=''
for _a in \"\$@\"; do
    [[ \"\${_want_w}\" == 'out' ]] && { _out_file=\"\${_a}\"; _want_w=''; continue; }
    [[ \"\${_a}\" == '-o' ]] && { _want_w='out'; continue; }
done
if [[ \"\${_c}\" -le 1 ]]; then
    [[ -n \"\${_out_file}\" ]] && printf '' > \"\${_out_file}\"
    printf '503'
    exit 0
else
    [[ -n \"\${_out_file}\" ]] && printf '{\"name\":\"ok\"}' > \"\${_out_file}\"
    printf '200'
    exit 0
fi
FAKECURL
    chmod +x \"\${_fake_dir}/curl\"
    # No-op sleep to keep the retry test fast (avoids real 5s back-off delays)
    printf '#!/bin/bash\nexit 0\n' > \"\${_fake_dir}/sleep\"
    chmod +x \"\${_fake_dir}/sleep\"
    out=\$(PATH=\"\${_fake_dir}:\${PATH}\" _gs_eu2_http_get_core 'https://example.com/test' 2>&1)
    rc=\$?
    [[ \"\${rc}\" -eq 0 ]] || { echo \"expected exit 0 after retry, got rc=\${rc}; out=\${out}\"; echo FAIL; exit 0; }
    printf '%s' \"\${out}\" | grep -q 'ok' || { echo \"expected success body; got: \${out}\"; echo FAIL; exit 0; }
    # Verify curl was called more than once (retry happened)
    _final_count=\$(cat \"\${_count_file}\")
    [[ \"\${_final_count}\" -gt 1 ]] || { echo \"expected >1 curl calls (retry), got \${_final_count}\"; echo FAIL; exit 0; }
    echo PASS
"

# t109b: _gs_eu2_http_get_core retries on 502 and fails after exhausting all attempts
t "t109b: http_get_core retries 502 and fails after exhausting 3 attempts" bash -c "
    source '/stack/bin/lib/env-update/http/curl.sh'
    # Create a fake curl that always returns 502.
...
_count_file=\"\$(dirname \"\$0\")/.call_count\"
_c=\"\$(cat \"\${_count_file}\" 2>/dev/null || echo 0)\"
printf '%d' \$(( _c + 1 )) > \"\${_count_file}\"
_out_file=''
_want_out=''
for _a in \"\$@\"; do
    [[ \"\${_want_out}\" == 'yes' ]] && { _out_file=\"\${_a}\"; _want_out=''; continue; }
    [[ \"\${_a}\" == '-o' ]] && { _want_out='yes'; continue; }
done
[[ -n \"\${_out_file}\" ]] && printf '' > \"\${_out_file}\"
printf '502'
exit 0
FAKECURL
    chmod +x \"\${_fake_dir}/curl\"
    # No-op sleep to keep the retry test fast
    printf '#!/bin/bash\nexit 0\n' > \"\${_fake_dir}/sleep\"
    chmod +x \"\${_fake_dir}/sleep\"
    rc=0
    out=\$(PATH=\"\${_fake_dir}:\${PATH}\" _gs_eu2_http_get_core 'https://example.com/test' 2>&1) || rc=\$?
    [[ \"\${rc}\" -ne 0 ]] || { echo \"expected non-zero exit after persistent 502, got 0; out=\${out}\"; echo FAIL; exit 0; }
    # Verify retry happened: curl should be called 3 times (one per outer attempt)
    _final_count=\$(cat \"\${_count_file}\")
    [[ \"\${_final_count}\" -ge 3 ]] || { echo \"expected >=3 curl calls (retry), got \${_final_count}\"; echo FAIL; exit 0; }
    # Verify stderr mentions 5xx transient retry
    printf '%s' \"\${out}\" | grep -qi 'transient\|5xx\|502\|retry' \
        || { echo \"expected retry mention in stderr; got: \${out}\"; echo FAIL; exit 0; }
    echo PASS
"

# t109c: 404 does NOT trigger 5xx retry — fast fail, curl called exactly once per outer attempt
t "t109c: 404 fails fast — no 5xx retry, curl called only once" bash -c "
    source '/stack/bin/lib/env-update/http/curl.sh'
...
FAKECURL
    chmod +x \"\${_fake_dir}/curl\"
    # No-op sleep (404 should not need it, but include for consistency)
    printf '#!/bin/bash\nexit 0\n' > \"\${_fake_dir}/sleep\"
    chmod +x \"\${_fake_dir}/sleep\"
    rc=0
    PATH=\"\${_fake_dir}:\${PATH}\" _gs_eu2_http_get_core 'https://example.com/test' 2>/dev/null || rc=\$?
    [[ \"\${rc}\" -ne 0 ]] || { echo 'expected non-zero exit for 404'; echo FAIL; exit 0; }
    _final_count=\$(cat \"\${_count_file}\")
    # 404 must NOT trigger retry — only 1 outer attempt (curl's --retry 3 is tcp-only)
    # The outer loop should break immediately since 404 != 429 and != 5xx
    [[ \"\${_final_count}\" -eq 1 ]] || { echo \"expected 1 curl call for 404 (no retry), got \${_final_count}\"; echo FAIL; exit 0; }
    echo PASS
"

# t109d: codeberg fetcher — releases 404 + transient 503 on tags → ERROR after retries
# (validates that Fix 1 makes the codeberg tags-fallback path retry-aware)
t "t109d: codeberg tags-fallback 503 → ERROR after retry exhaustion (Fix 2 via Fix 1)" bash -c "
    source '/stack/bin/lib/env-update/config/defaults.sh'
    source '/stack/bin/lib/env-update/config/prerelease_markers.sh'
    source '/stack/bin/lib/env-update/core/records.sh'
    source '/stack/bin/lib/env-update/core/semver.sh'
    ...
    source '/stack/bin/lib/env-update/http/curl.sh'
    source '/stack/bin/lib/env-update/fetchers/codeberg.sh'
    # Fake curl: always 503 (simulates complete outage on tags endpoint after releases 404)
...
[[ -n \"\${_out_file}\" ]] && printf '' > \"\${_out_file}\"
# Return 404 for releases endpoint, 503 for tags endpoint
...
FAKECURL
    chmod +x \"\${_fake_dir}/curl\"
    # No-op sleep to avoid real back-off delays during retry
    printf '#!/bin/bash\nexit 0\n' > \"\${_fake_dir}/sleep\"
    chmod +x \"\${_fake_dir}/sleep\"
    export _GS_EU2_CACHE_DIR=\"\${TMP_DIR}/t109d_cache\"
    _gs_eu2_record_new; idx=\${_GS_EU2_LAST_IDX}
    _gs_eu2_record_set \$idx type       'codeberg'
    _gs_eu2_record_set \$idx identifier 'testorg/tags-only'
    _gs_eu2_record_set \$idx env_var    'GLOBAL_STACK_TAGSONLY_VERSION'
