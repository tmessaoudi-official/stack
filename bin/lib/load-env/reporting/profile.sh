#!/bin/bash
# profile.sh — execution profiling for le_main phases

# Include guard
[[ -n "${_GS_LE_PROFILE_SH_LOADED:-}" ]] && return 0
readonly _GS_LE_PROFILE_SH_LOADED=1

# ── State variables (global) ──────────────────────────────────────────────────
declare -ag _GS_LE_PROFILE_PHASES=()
declare -ag _GS_LE_PROFILE_DURATIONS_MS=()
declare -ag _GS_LE_PROFILE_MEM_DELTAS_KB=()
_GS_LE_PROFILE_PHASE_START_MS=0
_GS_LE_PROFILE_PHASE_START_KB=0
_GS_LE_PROFILE_TOTAL_START_MS=0
_GS_LE_PROFILE_PEAK_KB=0
...
# ── _gs_le_profile_init: called once at the start of le_main ────────────────
_gs_le_profile_init() {
    _GS_LE_PROFILE_TOTAL_START_MS=$(_gs_le_now_ms)
    _GS_LE_PROFILE_PEAK_KB=$(_gs_le_rss_kb)
}

# ── _gs_le_profile_start: called before a phase begins ──────────────────────
_gs_le_profile_start() {
    _GS_LE_PROFILE_PHASE_START_MS=$(_gs_le_now_ms)
    _GS_LE_PROFILE_PHASE_START_KB=$(_gs_le_rss_kb)
}

# ── _gs_le_profile_end "Phase Name": called after a phase completes ──────────
_gs_le_profile_end() {
    local _name="$1"
    local _end_ms _end_kb
    _end_ms=$(_gs_le_now_ms)
    _end_kb=$(_gs_le_rss_kb)
    local _dur=$(( _end_ms - _GS_LE_PROFILE_PHASE_START_MS ))
    local _delta=$(( _end_kb - _GS_LE_PROFILE_PHASE_START_KB ))
    _GS_LE_PROFILE_PHASES+=("${_name}")
    _GS_LE_PROFILE_DURATIONS_MS+=("${_dur}")
    _GS_LE_PROFILE_MEM_DELTAS_KB+=("${_delta}")
    [[ ${_end_kb} -gt ${_GS_LE_PROFILE_PEAK_KB} ]] && _GS_LE_PROFILE_PEAK_KB=${_end_kb}
}
...
# ── _gs_le_profile_report: print the pretty summary table ────────────────────
_gs_le_profile_report() {
    ...
    local _total_ms=0
    local _now_ms
    _now_ms=$(_gs_le_now_ms)
    _total_ms=$(( _now_ms - _GS_LE_PROFILE_TOTAL_START_MS ))
    ...
    local _i
    for _i in "${!_GS_LE_PROFILE_PHASES[@]}"; do
        local _name="${_GS_LE_PROFILE_PHASES[${_i}]}"
        local _ms="${_GS_LE_PROFILE_DURATIONS_MS[${_i}]}"
        local _kb="${_GS_LE_PROFILE_MEM_DELTAS_KB[${_i}]}"
        ...
        local _dur_str
        _dur_str=$(_gs_le_fmt_duration "${_ms}")
        local _mem_str
        _mem_str=$(_gs_le_fmt_mem "${_kb}")
    ...
    _total_dur_str=$(_gs_le_fmt_duration "${_total_ms}")
    local _peak_mb_int=$(( _GS_LE_PROFILE_PEAK_KB / 1024 ))
    local _peak_mb_frac=$(( (_GS_LE_PROFILE_PEAK_KB % 1024) * 10 / 1024 ))
    local _peak_str
