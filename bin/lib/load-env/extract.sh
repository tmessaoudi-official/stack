#!/bin/bash
# extract.sh — le_search_and_extract + le_detect_multiple_defaults

# Include guard
[[ -n "${_LE_EXTRACT_SH_LOADED:-}" ]] && return 0
readonly _LE_EXTRACT_SH_LOADED=1
...
# Reads from _LE_CFG: all_src_env_merged_name, exclude_implicit_empty,
...
le_detect_multiple_defaults() {
	...
	awk -F'=' '
		...
		($1 in keys)
	' "${input_file}" "${_LE_CFG[all_src_env_merged_name]}" >> "${input_file_merge}"

	envsubst < "${input_file_merge}" > "${input_file_merge}.expanded"

	local multiple_default_values
	multiple_default_values=$(awk -F '=' -v exclude_implicit_empty="${_LE_CFG[exclude_implicit_empty]}" -v exclude_explicit_empty="${_LE_CFG[exclude_explicit_empty]}" '
	{
		key = $1;
	...
	if [[ -n "${_LE_CFG[exclude_multiple_values_pattern]}" && -n "${multiple_default_values}" ]]; then
		multiple_default_values=$(echo "${multiple_default_values}" | grep -vE "${_LE_CFG[exclude_multiple_values_pattern]}")
	fi

	if [[ -n "${multiple_default_values}" ]]; then
		echo -e "\n ---- (le_detect_multiple_defaults): Entries defined multiple times in ${current_file} with multiple values:\n${multiple_default_values}\n"
	fi
...
# ── le_search_and_extract ─────────────────────────────────────────────────────
# Args: current_file  count
# Reads from _LE_CFG: search_path_ignore_pattern, extract_all_exclude_pattern,
#                     debug, debug_show_extracted_files, include_docker_args,
#                     extract_all_prefix, extract_all_env_output_file,
#                     extract_all_env_delete_output, cleanup_tmp,
#                     all_src_env_merged_name, exclude_implicit_empty,
#                     exclude_explicit_empty
...
le_search_and_extract() {
	...
	local _ignore_re
	_ignore_re="$(printf '%s' "${_LE_CFG[search_path_ignore_pattern]}" | sed '/^\s*$/d' | paste -sd '|')"
	if [[ -n "${_ignore_re}" && "${current_file}" =~ ${_ignore_re} ]]; then
		if [[ "true" = "${_LE_CFG[debug]}" ]]; then
			echo -e "\n ---- (le_search_and_extract): Ignoring path: ${current_file}\n"
		fi
		return 0
	fi

	if [[ "true" = "${_LE_CFG[debug]}" && "true" = "${_LE_CFG[debug_show_extracted_files]}" ]]; then
		echo -e "\n ---- (le_search_and_extract): Extracting env variables from ${current_file}\n"
	fi

	# Per-file output goes into the session temp dir to avoid collisions
	local _out_file="${_LE_SESSION_TMP}/extract.${count}"
	...
	local _pref="${_LE_CFG[extract_all_prefix]}"

	# Pass 1: assignment / definition forms (whole-line, greedy)
	local _pat1=""
	[[ "true" = "${_LE_CFG[include_docker_args]}" ]] && \
	...
	{ grep -oE "${_pat1}" "${current_file}" 2>/dev/null || true
	  grep -oE "${_pat2}" "${current_file}" 2>/dev/null || true; } |
		...
		if [[ -n "${_LE_CFG[extract_all_exclude_pattern]}" ]]; then
			grep -vE "${_LE_CFG[extract_all_exclude_pattern]}"
		else
			cat
	...
	le_detect_multiple_defaults "${_out_file}" "${current_file}"

	cat "${_out_file}" >> "${_LE_CFG[extract_all_env_output_file]}"
	[[ "true" = "${_LE_CFG[extract_all_env_delete_output]}" && "true" = "${_LE_CFG[cleanup_tmp]}" ]] && rm -rf "${_out_file}"
}

# ── _le_run_extraction ────────────────────────────────────────────────────────
# Parallel find-and-extract loop, then consolidate results.
_le_run_extraction() {
	true > "${_LE_CFG[extract_all_env_output_file]}"

	if [[ -f "${_LE_CFG[search_path]}" ]]; then
		le_search_and_extract "${_LE_CFG[search_path]}" 0
	elif [[ -d "${_LE_CFG[search_path]}" ]]; then
		local count=0
		local -a pids=()

		while IFS= read -r _file; do
			((count++))
			le_search_and_extract "${_file}" "${count}" &
			pids+=($!)
		done < <(find "${_LE_CFG[search_path]}" -type f)
		# \( \
		# 	-name 'Dockerfile' \
		# 	-o -name '*.yaml' \
		# 	-o -name '*.sh' \
		...
		[[ "${failed}" -eq 0 ]] || { echo "le_search_and_extract: ${failed} background job(s) failed" >&2; exit 1; }
	else
		echo -e "\n ---- (le_main): ${_LE_CFG[search_path]} is neither a file nor a directory, exiting !\n\n" >&2
		exit 1
	fi
	...
	LC_ALL=C sort -u "${_LE_CFG[extract_all_env_output_file]}" \
		-o "${_LE_CFG[extract_all_env_output_file]}" 2>/dev/null || true
}
