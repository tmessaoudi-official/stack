...
# Include guard
[[ -n "${_GS_LE_EXTRACT_SH_LOADED:-}" ]] && return 0
readonly _GS_LE_EXTRACT_SH_LOADED=1
...
# Reads from _GS_LE_CFG: source_merged_file, exclude_implicit_empty,
...
le_detect_multiple_defaults() {
	...
	awk -F'=' '
		...
		($1 in keys)
	' "${input_file}" "${_GS_LE_CFG[source_merged_file]}" >> "${input_file_merge}"

	envsubst < "${input_file_merge}" > "${input_file_merge}.expanded"

	local multiple_default_values
	multiple_default_values=$(awk -F '=' -v exclude_implicit_empty="${_GS_LE_CFG[exclude_implicit_empty]}" -v exclude_explicit_empty="${_GS_LE_CFG[exclude_explicit_empty]}" '
	{
		key = $1;
	...
	if [[ -n "${_GS_LE_CFG[exclude_multiple_values_pattern]}" && -n "${multiple_default_values}" ]]; then
		multiple_default_values=$(echo "${multiple_default_values}" | grep -vE "${_GS_LE_CFG[exclude_multiple_values_pattern]}")
	fi
...
# Reads from _GS_LE_CFG: scan_ignore_pattern, scan_exclude_pattern,
...
#                        exclude_explicit_empty
# Session temp dir: _GS_LE_SESSION_TMP (set by le_main)
le_search_and_extract() {
	...
	local _ignore_re
	_ignore_re="$(printf '%s' "${_GS_LE_CFG[scan_ignore_pattern]}" | sed '/^\s*$/d' | paste -sd '|')"
	if [[ -n "${_ignore_re}" && "${current_file}" =~ ${_ignore_re} ]]; then
		if [[ "true" = "${_GS_LE_CFG[debug]}" ]]; then
			echo -e "\n ---- (le_search_and_extract): Ignoring path: ${current_file}\n"
		fi
		return 0
	fi

	if [[ "true" = "${_GS_LE_CFG[debug]}" && "true" = "${_GS_LE_CFG[debug_show_extracted_files]}" ]]; then
	...
	local _out_file="${_GS_LE_SESSION_TMP}/extract.${count}"
	...
	local _pref="${_GS_LE_CFG[scan_var_prefix]}"

	# Pass 1: assignment / definition forms (whole-line, greedy)
	local _pat1=""
	[[ "true" = "${_GS_LE_CFG[include_docker_args]}" ]] && \
	...
	{ grep -oE "${_pat1}" "${current_file}" 2>/dev/null || true
	  grep -oE "${_pat2}" "${current_file}" 2>/dev/null || true; } |
		...
		if [[ -n "${_GS_LE_CFG[scan_exclude_pattern]}" ]]; then
			grep -vE "${_GS_LE_CFG[scan_exclude_pattern]}"
		else
			cat
	...
	cat "${_out_file}" >> "${_GS_LE_CFG[scan_output_file]}"
	[[ "true" = "${_GS_LE_CFG[scan_delete_output]}" && "true" = "${_GS_LE_CFG[cleanup_tmp]}" ]] && rm -rf "${_out_file}"
}

# ── _le_run_extraction ────────────────────────────────────────────────────────
# Parallel find-and-extract loop, then consolidate results.
_le_run_extraction() {
	true > "${_GS_LE_CFG[scan_output_file]}"

	if [[ -f "${_GS_LE_CFG[scan_path]}" ]]; then
		le_search_and_extract "${_GS_LE_CFG[scan_path]}" 0
	elif [[ -d "${_GS_LE_CFG[scan_path]}" ]]; then
		local count=0
		local -a pids=()
		...
		while IFS= read -r _file; do
			...
			pids+=($!)
		done < <(find "${_GS_LE_CFG[scan_path]}" -type f)
		...
		echo -e "\n ---- (le_main): ${_GS_LE_CFG[scan_path]} is neither a file nor a directory, exiting !\n\n" >&2
		exit 1
	fi
	...
	LC_ALL=C sort -u "${_GS_LE_CFG[scan_output_file]}" \
		-o "${_GS_LE_CFG[scan_output_file]}" 2>/dev/null || true
}
