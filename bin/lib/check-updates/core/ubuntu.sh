...
_GS_CU_UBUNTU_ENV_CODENAME=""
_GS_CU_UBUNTU_ENV_VERSION=""
...
_gs_cu_ubuntu_init() {
  ...
  if [[ -n "${ubuntu_line}" ]]; then
    _GS_CU_UBUNTU_ENV_VERSION="${ubuntu_line#*=}"
    _GS_CU_UBUNTU_ENV_CODENAME="$(_gs_cu_extract_codename_from_version "${_GS_CU_UBUNTU_ENV_VERSION}")"
  fi
  _gs_cu_log_debug "Ubuntu env codename: ${_GS_CU_UBUNTU_ENV_CODENAME} (from ${_GS_CU_UBUNTU_ENV_VERSION})"
}

# Check if a Docker Hub tag exists for the given image+tag.
# Returns 0 if it exists, 1 if not.
_gs_cu_ubuntu_tag_exists_on_dockerhub() {
...
_gs_cu_ubuntu_fetch_latest_ubuntu_image() {
  local current_version="${1}"
  local env_codename="${2}"

  # Fetch all tags for _/ubuntu matching the env codename
  local tags_json
  if ! tags_json="$(_gs_cu_dockerhub_fetch_tags "_" "ubuntu" 2>/dev/null)"; then
    return 1
  fi
  ...
  if [[ -z "${matching_tag}" ]]; then
    _gs_cu_log_debug "No dated tag found for ubuntu:${env_codename}-YYYYMMDD"
    return 1
  fi
...
# Usage: _gs_cu_ubuntu_align_codename_in_version "8.2.6-rc0-noble" "namespace" "image"
# Returns: echoes proposed version or original if no change needed
_gs_cu_ubuntu_align_codename_in_version() {
  local version="${1}"
  local namespace="${2}"
  local image="${3}"

  if [[ -z "${_GS_CU_UBUNTU_ENV_CODENAME}" ]]; then
    echo "${version}"
    return 0
  fi
  ...
  local ver_codename
  ver_codename="$(_gs_cu_extract_codename_from_version "${version}")"
  ...
  if [[ "${ver_codename}" == "${_GS_CU_UBUNTU_ENV_CODENAME}" ]]; then
    echo "${version}"
    return 0
  fi

  # Check if env codename is actually newer than ver_codename
  if ! _gs_cu_codename_is_older "${ver_codename}" "${_GS_CU_UBUNTU_ENV_CODENAME}"; then
    echo "${version}"
    return 0
  fi

  # Build proposed version with env codename
  local proposed_version="${version//${ver_codename}/${_GS_CU_UBUNTU_ENV_CODENAME}}"

  # Check if this tag exists on Docker Hub
  if _gs_cu_ubuntu_tag_exists_on_dockerhub "${namespace}" "${image}" "${proposed_version}"; then
...
_gs_cu_ubuntu_process_record() {
  ...
  local ver_codename
  ver_codename="$(_gs_cu_extract_codename_from_version "${current_version}")"
  ...
  local proposed_version="${current_version//${ver_codename}/${_GS_CU_UBUNTU_ENV_CODENAME}}"
