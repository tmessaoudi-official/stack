...
if eval "${env_file_exists}"; then
  _gs_unu_exclude="^(COMPOSE_FILE|BUILDX_EXPERIMENTAL|BUILDKIT_PROGRESS|BUILDX_BUILDER|COMPOSE_PROJECT_NAME|COMPOSE_REMOVE_ORPHANS|COMPOSE_HTTP_TIMEOUT|GLOBAL_STACK_EXPOSED_VIRTUAL_HOSTS|COMPOSE_PATH_SEPARATOR|DOCKER_BUILDKIT|COMPOSE_DOCKER_CLI_BUILD|GLOBAL_STACK_COMPOSE_CLI|GLOBAL_STACK_HTTPS_LOCALHOST_IPS|GLOBAL_STACK_HTTPS_CONTAINER_IPS|GLOBAL_STACK_(.*)_COMMAND_SUFFIX|GLOBAL_STACK_(.*)_CLI_OPTIONS|GLOBAL_STACK_(.*)_CLI_VARIANTS|GLOBAL_STACK_ANDROID_SYSTEM_IMAGES|GLOBAL_STACK_ANDROID_PACKAGES)="
  _gs_unu_envfile="${GLOBAL_STACK_DOCKER_ROOT_PATH}/.env.local"
  # Triple eval: each pass exports the full file; vars referencing later-defined vars
  # resolve correctly on the 2nd/3rd pass once deps are in the environment.
  # eval+sed is required (not source): .env.local has unquoted multi-word values
  # (e.g. FULL_NAME=Foo Bar) that source would execute as commands.
  eval "$(grep -vE "${_gs_unu_exclude}" "${_gs_unu_envfile}" | sed 's/^/export /')"
  eval "$(grep -vE "${_gs_unu_exclude}" "${_gs_unu_envfile}" | sed 's/^/export /')"
  eval "$(grep -vE "${_gs_unu_exclude}" "${_gs_unu_envfile}" | sed 's/^/export /')"
  unset _gs_unu_exclude _gs_unu_envfile
fi
