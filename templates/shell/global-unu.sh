...
if eval "${env_file_exists}"; then
  	eval $(grep -vE "^(COMPOSE_FILE|BUILDX_EXPERIMENTAL|COMPOSE_PROJECT_NAME|COMPOSE_REMOVE_ORPHANS|COMPOSE_HTTP_TIMEOUT|GLOBAL_STACK_EXPOSED_VIRTUAL_HOSTS|COMPOSE_PATH_SEPARATOR|DOCKER_BUILDKIT|COMPOSE_DOCKER_CLI_BUILD|GLOBAL_STACK_COMPOSE_CLI|GLOBAL_STACK_POSTGRES17_DBS|GLOBAL_STACK_HTTPS_LOCALHOST_IPS|GLOBAL_STACK_(.*)_COMMAND_SUFFIX)=" "${GLOBAL_STACK_DOCKER_ROOT_PATH}"/.env.local | sed 's/^/export /')
  	eval $(grep -vE "^(COMPOSE_FILE|BUILDX_EXPERIMENTAL|COMPOSE_PROJECT_NAME|COMPOSE_REMOVE_ORPHANS|COMPOSE_HTTP_TIMEOUT|GLOBAL_STACK_EXPOSED_VIRTUAL_HOSTS|COMPOSE_PATH_SEPARATOR|DOCKER_BUILDKIT|COMPOSE_DOCKER_CLI_BUILD|GLOBAL_STACK_COMPOSE_CLI|GLOBAL_STACK_POSTGRES17_DBS|GLOBAL_STACK_HTTPS_LOCALHOST_IPS|GLOBAL_STACK_(.*)_COMMAND_SUFFIX)=" "${GLOBAL_STACK_DOCKER_ROOT_PATH}"/.env.local | sed 's/^/export /')
  	eval $(grep -vE "^(COMPOSE_FILE|BUILDX_EXPERIMENTAL|COMPOSE_PROJECT_NAME|COMPOSE_REMOVE_ORPHANS|COMPOSE_HTTP_TIMEOUT|GLOBAL_STACK_EXPOSED_VIRTUAL_HOSTS|COMPOSE_PATH_SEPARATOR|DOCKER_BUILDKIT|COMPOSE_DOCKER_CLI_BUILD|GLOBAL_STACK_COMPOSE_CLI|GLOBAL_STACK_POSTGRES17_DBS|GLOBAL_STACK_HTTPS_LOCALHOST_IPS|GLOBAL_STACK_(.*)_COMMAND_SUFFIX)=" "${GLOBAL_STACK_DOCKER_ROOT_PATH}"/.env.local | sed 's/^/export /')
fi
...
if eval "${env_file_exists}"; then
	if [ "" != "${OPERATING_SYSTEM}" ]; then
		...
		if [[ "" != "${TASK_ARCH}" && -n "${GLOBAL_STACK_TASK_VERSION}" && "" != "${GLOBAL_STACK_TASK_VERSION}" ]]; then
			...
			if [ "${GLOBAL_UNU_TASK_LATEST}" != "${GLOBAL_UNU_TASK_VERSION}" ]; then
