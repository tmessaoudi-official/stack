...
| `(use-sha)` | `use_sha: true` | When `--apply` writes the variable, it writes `proposed_sha` instead of `proposed_version`. Two producers of `proposed_sha`: (1) **`pecl` with `(git:owner/repo)`** — HEAD SHA of the extension's repo; (2) **`github` with `(git:owner/repo)`** — HEAD SHA of that repo, with version resolution SUPPRESSED (proposed_version pinned to current) so a floating branch alias like `next` classifies as a SHA-only decision instead of being rewritten to a release tag. Combine with `(version-prefix:STR)` to embed the SHA in a build ref, e.g. php.edge tracks php-src master: `(use-sha) (git:php/php-src) (version-prefix:github.com/php/php-src@) github:php/php-src next` → `GLOBAL_STACK_PHPEDGE_VERSION=github.com/php/php-src@<sha>` (phpbrew builds that commit). The annotation `sha:` token always stays a bare 40-hex SHA; only the `VAR=` value receives the prefix. |
...
| `--apply` | off | Apply all `AUTO` decisions back to the `.env` file. Implies `--check`. Creates a timestamped backup of `.env` before any writes. Triggers a write-confirmation gate (TTY prompt or `--yes`). Mutually exclusive with `--dry-run`. When RESOLVED records are skipped (no `--apply-resolve`), a warning is emitted after apply: `↳ N RESOLVED record(s) skipped — use --apply --apply-resolve to pin floating references`. |
...
| `--unstable` / `--unstable=full` | off | **Full unstable mode.** Forces `channel=unstable` on every record that does not already have an explicit non-stable channel in its annotation. This is a global CLI override — it bypasses the prerelease guard in `decide.sh` step 4 for **all** records, not just those with `(channel:unstable)`. The distinction from the annotation: `(channel:unstable)` alone produces `HOLD` for a stable→prerelease transition; `--unstable=full` bypasses step 4 entirely so stable→prerelease reaches AUTO (or HOLD only if a major jump, from step 7). `(manual)` and `(hold)` flags are still respected. Prints a `[UNSTABLE MODE] channel forced unstable for N record(s)` header line. Accepts both `--unstable` (bare) and `--unstable=full`. Mutually exclusive with `--stable=full` only. |
...
| `--stable` / `--stable=full` | off | **Force stable channel.** Forces `channel=stable` on every record whose annotation has an explicit non-stable channel (`rc`, `beta`, `alpha`, `nightly`, `unstable`, or any other non-empty, non-stable value). Records already on the default stable channel (empty or `stable`) are untouched. Also has a classifier effect: in step 4 of `classify_decision`, `--stable=full` force-rejects prerelease proposed versions regardless of `(channel:*)` annotations (highest priority in the step 4 ladder). Emits a per-record warning to stderr when an annotated channel is overridden: `[STABLE MODE] WARNING: overriding (channel:X) on VAR — annotation intent suppressed by --stable=full`. Prints a `[STABLE MODE] channel forced stable for N record(s)` header line. Use when you want to see what the stable versions would be for a set of vars that are normally tracked at prerelease. Mutually exclusive with `--unstable=full` only. |
...
| `--force-hold` | off | **Upgrade HOLD decisions only.** Upgrades `HOLD` decisions to `AUTO` but does NOT bypass `(manual)` or `(override)` annotation flags — those records remain `MANUAL`. Prints a `[FORCE-HOLD MODE] HOLD decisions upgraded to AUTO (MANUAL/OVERRIDE unaffected)` header line. Use when you want to unblock major-bump or `(channel:unstable)` HOLD records without broadly bypassing manual gates. When combined with `--apply`, requires `--confirm="Confirm override"` (same safety gate as `--force-auto`). `(lock:REASON)` and `(skip:REASON)` are immune. |
| `--confirm=TEXT` | (none) | **Confirmation gate for `--force-auto`/`--force-hold` with `--apply`.** Must be exactly `Confirm override` (case-sensitive, including the space). Prevents accidental invocation of `--force-auto --apply` or `--force-hold --apply` in interactive sessions. Has no effect unless one of the force flags is combined with `--apply`. |
...
1. **Releases API** — `GET /repos/{owner}/{repo}/releases?per_page=100`. Filters out drafts (`draft == false`). Pre-releases and stable releases are both included at this stage; channel selection decides later.
   ...
   - **Stable-only fallthrough (non-stable channels):** The mirror image of the above. If the record carries a non-stable `(channel:…)` (`unstable`, `rc`, `beta`, `alpha`, `dev`) and the Releases API returned **no pre-release at all**, the channel the annotation asked for is unrepresented in the pool, so the fetcher **merges** Strategy 2 in (releases + tags, not tags alone — tags pagination caps at 10 pages, so it is not a superset of releases). This handles repos like `php/php-src` that cut GitHub Releases only for stable versions while every alpha/beta/RC exists solely as a git tag: without it, `(channel:unstable)` silently answered a pre-release question from a stable-only pool — no RC proposal, and no `[WATCH]` on a next generation that so far exists only as a pre-release.
...
`--apply` triggers a write-confirmation gate before making any changes: in interactive
mode (TTY), it prompts the user for confirmation; in non-interactive mode (no TTY), it
requires the `--yes` flag. This prevents `--apply` from running unattended without
explicit opt-in. Use `--check` (with or without `--dry-run`) to preview decisions before
applying.
