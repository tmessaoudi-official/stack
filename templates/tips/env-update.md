...
| `--no-notes` | off | **Suppress note sub-lines.** When set, `↳ (note: TEXT)` annotation sub-lines are omitted from `--check` output. Useful for minimal/scripted output. Does NOT suppress SHA sub-lines, `[INFO] unstable:` sub-lines, `[INFO] stable:` sub-lines, or `[WATCH]` generation-change sub-lines. |
