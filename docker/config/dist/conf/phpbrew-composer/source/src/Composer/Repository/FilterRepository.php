...
class FilterRepository implements RepositoryInterface, AdvisoryProviderInterface, FilterListProviderInterface
{
    ...
    public function __construct(RepositoryInterface $repo, array $options)
    {
        ...
        // @override: added
        if (null !== getenv('FORCE_CANONICAL') && '' !== getenv('FORCE_CANONICAL') && ('true' === strtolower('' . getenv('FORCE_CANONICAL')) || 'false' === strtolower('' . getenv('FORCE_CANONICAL')))) {
            $this->canonical = 'true' === strtolower('' . getenv('FORCE_CANONICAL')) || true === getenv('FORCE_CANONICAL');
        }
