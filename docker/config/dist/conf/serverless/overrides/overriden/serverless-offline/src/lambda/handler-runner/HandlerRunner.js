...
export default class HandlerRunner {
  ...
  async #loadRunner() {
    ...
    // @override
    if (supportedPhp.has(runtime)) {
      const { default: PhpRunner } = await import("./php-runner/index.js")

      return new PhpRunner(this.#funOptions, this.#env)
    }

    // @override
    if (supportedRust.has(runtime)) {
      const { default: RustRunner } = await import("./rust-runner/index.js")

      return new RustRunner(this.#funOptions, this.#env)
    }
