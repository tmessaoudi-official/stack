...
export default class RustRunner {
  ...
  #parsePayload(value) {
    ...
    for (const item of value.split(EOL)) {
      ...
      if (
        json &&
        typeof json === "object" &&
        hasOwn(json, RustRunner.#payloadIdentifier)
      ) {
        payload = json[RustRunner.#payloadIdentifier]
      } else {
        log.notice(item)
      }
    }
  ...
  async run(event, context) {
    const runtime = platform() === "win32" ? "rustc.exe" : "rustc"
    ...
    const { stderr, stdout } = await execa(
      runtime,
      [
        join(import.meta.url, "invoke.rs"),
