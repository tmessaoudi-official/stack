...
export default class PhpRunner {
  ...
  #parsePayload(value) {
    ...
    for (const item of value.split(EOL)) {
      ...
      if (
        json &&
        typeof json === "object" &&
        hasOwn(json, PhpRunner.#payloadIdentifier)
      ) {
        payload = json[PhpRunner.#payloadIdentifier]
  ...
  async run(event, context) {
    const runtime = platform() === "win32" ? "php.exe" : "php"
    ...
    const { stderr, stdout } = await execa(
      runtime,
      [
        join(import.meta.url, "invoke.php"),
