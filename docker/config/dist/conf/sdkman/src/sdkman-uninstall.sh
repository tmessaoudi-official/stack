...
function __sdk_uninstall() {
	...
	# @changed stack
	current=$(readlink "${HOME}/.sdkman/${candidate}/current" | sed "s!${SDKMAN_CANDIDATES_DIR}/${candidate}/!!g")
	#if [[ -L "${SDKMAN_CANDIDATES_DIR}/${candidate}/current" && "$version" == "$current" ]]; then
	# @changed stack
	if [[ -L "${HOME}/.sdkman/${candidate}/current" && "$version" == "$current" ]]; then
		echo ""
		__sdkman_echo_green "Deselecting ${candidate} ${version}..."
		#unlink "${SDKMAN_CANDIDATES_DIR}/${candidate}/current"
		# @changed stack
		unlink "${HOME}/.sdkman/${candidate}/current"
	fi
