...
function __sdkman_determine_version() {
	...
	# @changed stack
	elif [[ "$SDKMAN_AVAILABLE" == "false" && -z "$version" && -L "${HOME}/.sdkman/${candidate}/current" ]]; then
		VERSION=$(readlink "${HOME}/.sdkman/${candidate}/current" | sed "s!${SDKMAN_CANDIDATES_DIR}/${candidate}/!!g")
