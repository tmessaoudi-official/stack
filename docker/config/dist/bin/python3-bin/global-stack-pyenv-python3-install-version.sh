...
stackCatch() {
  if [ "${1}" != "0" ]; then
    ...
    echo "Error detected !!"
    echo -e "$(date '+%d-%m-%Y %H:%M:%S'): Error - ** line: ${2} ** ** message: ${3} ** pyenv (${PYTHON_VERSION_AS:-${PYTHON_VERSION:-}}) ${PYENV_MODE:-} global-stack-pyenv-python3-install-version.sh" >> "${GLOBAL_STACK_DOCKER_TOOLS_PATH}/elapsed"
