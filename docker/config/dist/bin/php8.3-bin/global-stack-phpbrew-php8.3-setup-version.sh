...
stackCatch() {
  if [ "${1}" != "0" ]; then
    ...
    echo "Error detected !!"
    echo -e "\n$(date '+%d-%m-%Y %H:%M:%S'): Error - ** line: ${2} ** ** message: ${3} ** phpbrew (${PHPBREW_PHP_FINAL_VERSION}) ${PHPBREW_MODE:-} global-stack-phpbrew-php8.3-setup-version.sh" >> "${GLOBAL_STACK_DOCKER_TOOLS_PATH}/elapsed"
    sleep infinity
  fi
}
...
# LD_LIBRARY_PATH="$(php-config --prefix)/lib:${LD_LIBRARY_PATH}" CGO_CFLAGS="$(php-config --includes) -g -O0" CGO_LDFLAGS="-L$(php-config --prefix)/lib $(php-config --ldflags) $(php-config --libs) -g" go build -ldflags="-v" -o ${PHPBREW_BIN}/frankenphp-${GLOBAL_STACK_FRANKENPHP_VERSION}-${ACTUAL_PHP_VERSION} || true
LD_LIBRARY_PATH="$(php-config --prefix)/lib:${LD_LIBRARY_PATH}" CGO_CFLAGS="$(php-config --includes)" CGO_LDFLAGS="-L$(php-config --prefix)/lib $(php-config --ldflags) $(php-config --libs)" go build -ldflags="-v" -o ${PHPBREW_BIN}/frankenphp-${GLOBAL_STACK_FRANKENPHP_VERSION}-${ACTUAL_PHP_VERSION} || true
