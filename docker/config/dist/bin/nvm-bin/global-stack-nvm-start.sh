...
PATH="${GLOBAL_STACK_DOCKER_TOOLS_PATH}/yarn/bin:${DENO_DIR}/bin:${PNPM_HOME}:${PATH}"
export PATH
...
echo "PATH=${GLOBAL_STACK_DOCKER_TOOLS_PATH}/yarn/bin:${DENO_DIR}/bin:${PNPM_HOME}:${PATH}" >> "/home/${GLOBAL_STACK_DOCKER_USER_ID}/${GLOBAL_STACK_SHELL_RC_TARGET}"
...
if [ "${NVM_MODE}" = "install" ]; then
  ...
  if [ "${GLOBAL_STACK_RELOAD_NVM}" = "true" ]; then
    ...
    mkdir -p "${NVM_DIR}" "${DENO_INSTALL}/bin" "${YARN_OFFLINE_MIRROR}" "${YARN_CACHE_FOLDER}" "${YARN_GLOBAL_FOLDER}/bin" "${GLOBAL_STACK_PNPM_GLOBAL_DIR}" "${PNPM_HOME}" "${GLOBAL_STACK_PNPM_STORE_DIR}" "${NPM_CACHE_DIR}"
  fi
fi
