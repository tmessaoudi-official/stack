...
stackCatch() {
  if [ "${1}" != "0" ]; then
    ...
    echo "Error detected !!"
    echo -e "$(date '+%d-%m-%Y %H:%M:%S'): Error - ** line: ${2} ** ** message: ${3} ** nvm (${NODE_VERSION_AS:-${NODE_VERSION:-}}) ${NVM_MODE:-} global-stack-nvm-node24-setup-project.sh" >> "${GLOBAL_STACK_DOCKER_TOOLS_PATH}/elapsed"
