...
stackCatch() {
  if [ "${1}" != "0" ]; then
    ...
    echo "Error detected !!"
    echo -e "\n$(date '+%d-%m-%Y %H:%M:%S'): Error - ** line: ${2} ** ** message: ${3} ** phpbrew ($([[ -n "${PHP_VERSION_AS:-}" && "" != "${PHP_VERSION_AS:-}" ]] && echo "${PHP_VERSION_AS:-}" || echo "${PHP_VERSION:-}")) ${PHPBREW_MODE:-} global-stack-phpbrew-php8.4-setup-version.sh" >> "${GLOBAL_STACK_DOCKER_TOOLS_PATH}/elapsed"
    sleep infinity
  fi
}
...
LD_LIBRARY_PATH="$(php-config --lib-dir):${LD_LIBRARY_PATH}" CGO_CFLAGS=$(php-config --includes) CGO_LDFLAGS="-L$(php-config --lib-dir) $(php-config --ldflags) $(php-config --libs)" go build -tags=nobadger,nowatcher -ldflags="-v" -o ${PHPBREW_BIN}/frankenphp-${PHP_VERSION} || true
