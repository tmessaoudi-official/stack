...
LD_LIBRARY_PATH="$(php-config --lib-dir):${LD_LIBRARY_PATH}" CGO_CFLAGS=$(php-config --includes) CGO_LDFLAGS="-L$(php-config --lib-dir) $(php-config --ldflags) $(php-config --libs)" go build -ldflags="-v" -o ${PHPBREW_BIN}/frankenphp-${PHP_VERSION} || true
