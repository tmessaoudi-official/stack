...
# The marker is composite (tranche 3 step 22, §74): the four plugin pins are built into
# the binary, so a plugin bump alone must rebuild caddy too — it did nothing before.
# xcaddy is the builder, not an input: the binary is determined by caddy + plugin pins.
_caddy_want="${GLOBAL_STACK_CADDY_VERSION};transform-encoder=${GLOBAL_STACK_CADDY_TRANSFORM_ENCODER_VERSION};brotli=${GLOBAL_STACK_CADDY_BROTLI_VERSION};security=${GLOBAL_STACK_CADDY_SECURITY_VERSION};cache-handler=${GLOBAL_STACK_CADDY_CACHE_HANDLER_VERSION}"
_caddy_gate="$(gs_version_gate "${CADDY_VERSIONS_PATH}" "${_caddy_want}" "caddy")"
...
# Build caddy if necessary. The iou builds and checks the new binary in a temp dir and
# only then replaces bin/caddy, so nothing is wiped here: vhosts/ and the Caddyfile are
# regenerated by caddy-setup on every boot, and logs/ is kept. The marker follows the
# iou's success. (The iou used to be handed the marker path twice.)
if [[ "${_caddy_gate}" != "skip" || "${GLOBAL_STACK_RELOAD_CADDY}" == "true" ]]; then
  global-stack-caddy-iou.sh \
    "${CADDY_PATH}"
  printf '%s\n' "${_caddy_want}" >"${CADDY_VERSIONS_PATH}"
fi
