...
echo "deb [signed-by=/etc/apt/trusted.gpg.d/podman.gpg arch=$(dpkg --print-architecture)] https://download.opensuse.org/repositories/devel:/kubic:/libcontainers:/${GLOBAL_STACK_PODMAN_CHANEL}/ /" | sudo tee /etc/apt/sources.list.d/podman.list
sudo apt-get update --allow-releaseinfo-change
